...
parser.add_argument('--decay_rate', type=float, default=0.2, help='Decay rate for lr decay [default: 0.7]')
parser.add_argument('--radius', type=float, default=3, help='Radius [default: 2]')
parser.add_argument('--sigma', type=float, default=2.00, help='BANDWIDTH [default: 1.00]')
...
os.system('cp train_feature_context_dfc_n128.py %s' % (LOG_DIR)) # bkp of model def
...
setproctitle("lx_dfc_exp%s" % FLAGS.exp_no)
...
def get_batch(dataset, index, npoints = NUM_POINT):
    ...
    if(dataset =='train'):
        ...
        if index>=len(train_xyz):
            index = index - len(train_xyz)
        while train_xyz[index].shape[0]<2048:
            index = np.random.choice(len(train_xyz),1)[0]
        
        trainSet = train_xyz[index]
        point_set =  trainSet[:,:3] - np.min(trainSet[:,:3], axis=0)
        trainFeats = train_feats[index]
        semantic_seg = convert_label(train_label[index].astype(np.int32))
...
#         sample_weight *= mask
    ...
    if(dataset == 'test'):
        ...
        cur_semantic_seg = convert_label(test_label[index].astype(np.int32))
...
def get_batch_wdp(dataset, batch_idx):
    ...
    for i in range(bsize):
        ps,seg,smpw,feat = get_batch(dataset,batch_idx*BATCH_SIZE+i)
        if np.random.random() >= 0.65:
#             ps = provider.jitter_point_cloud(ps.reshape(1, *ps.shape), sigma=0.01, clip=0.01)
...
#         batch_feats[i,drop_idx] = batch_feats[i,0]
...
NUM_CLASSES = 6
label_values = range(1,NUM_CLASSES) #ignore unknown

train_f = open('./data/train_1581.pickle', 'rb')
train_xyz, train_label, train_feats = pickle.load(train_f) #,encoding='iso-8859-1'
train_f.close()

test_f = open('./data/test_158.pickle', 'rb')
test_xyz, test_label, test_feats = pickle.load(test_f)
test_f.close()


label_w = convert_label(np.concatenate(train_label))
label_w = label_w[label_w>0]
tmp,_ = np.histogram(label_w,range(1,NUM_CLASSES+1))
labelweights = tmp
labelweights = labelweights.astype(np.float32)
labelweights = labelweights/np.sum(labelweights)
labelweights = 1/np.log(1.1+labelweights)
labelweights = np.insert(labelweights, 0, 1)
print labelweights

labelweights_t = np.ones(NUM_CLASSES)
print labelweights_t

# sampleweights = np.ones(label_w.shape)
...
#         print('lx train debug', aug_data.shape, batch_label.shape)
...
#         if aug_data.shape[1]<64:
#             print('lx eval pass', aug_data.shape, batch_label.shape)
#             continue
...
#         pred_val_new = np.argmax(pred_val[:,:,1:], 2)+1 #remove ignore label
        pred_val = np.argmax(pred_val, 2) #(B,N,6)
...
#     log_string('eval avg class acc: %f' % (np.mean(np.array(total_correct_class)/np.array(total_seen_class,dtype=np.float))))
    ...
    mloss = loss_sum / float(num_batches)
    se_loss_avg = loss_se / float(num_batches)
    macc = total_correct / float(total_seen)
...
# print sigma
pred, end_points, se_pred = MODEL.get_model(pointclouds_pl, is_training_pl, NUM_CLASSES, BANDWIDTH, bn_decay=bn_decay, features=feature_pl, radius=FLAGS.radius, dp=FLAGS.dropout, code=FLAGS.codeword)
...
# saver.restore(sess, 'log_lx_pointconv_ctx_dfc_n128_new/v_38/best_model_epoch_mf1_144.ckpt')
...
#     log_string('**** EPOCH %03d ****' %b (epoch))
    ...
    if (epoch<3) or (epoch%5==0 and epoch<=100) or (epoch%2==0 and epoch>100):
